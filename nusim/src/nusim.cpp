...
///
/// This simulator maintains ground truth pose and wheel states, publishes sensor data and
/// transforms, and responds to wheel command inputs.
///
/// @details Publishers:
///   - `~/timestep` (std_msgs::msg::UInt64): Current simulation timestep counter
///   - `~/real_walls` (visualization_msgs::msg::MarkerArray): Arena boundary walls for
///   visualization
///   - `~/real_obstacles` (visualization_msgs::msg::MarkerArray): Cylindrical obstacles for
///   visualization
///   - `red/sensor_data` (nuturtlebot_msgs::msg::SensorData): Simulated sensor output (ie encoder
...
public:
  ...
  : Node("nusimulator"), count_(0), gt_pose_()
  {
    ...
    }
    {
      auto desc = rcl_interfaces::msg::ParameterDescriptor();
      desc.description =
        "If true, draw the landmarks and do nothing else (for when connected to the actual robot)";
    ...
    }
    {
      auto desc = rcl_interfaces::msg::ParameterDescriptor();
      desc.description =
        "Wheel slip fraction -- bounds of uniform distribution to add slip noise proportional to "
        "wheel velocity";
    ...
    }
    {
      auto desc = rcl_interfaces::msg::ParameterDescriptor();
      desc.description =
        "Variance of zero-mean Gaussian noise to add to obstacle sensor measurements";
    ...
    if (draw_only_) {
      RCLCPP_INFO(
        get_logger(),
        "Draw only mode enabled: nusimulator will only publish the arena walls and obstacles; and "
        "will not simulate robot movement or publish sensor data.");
    } else {
      RCLCPP_INFO(
        get_logger(),
        "Simulation mode enabled: nusimulator will simulate robot movement and publish sensor "
        "data.");
      ...
      reset_service_ = create_service<std_srvs::srv::Empty>(
        "~/reset",
        std::bind(
          &NUSimulator::reset_callback, this, std::placeholders::_1, std::placeholders::_2));
      ...
      fake_sensor_timer_ =
        create_wall_timer(200ms, std::bind(&NUSimulator::fake_sensor_callback, this));
    }

    tf_broadcaster_ = std::make_unique<tf2_ros::TransformBroadcaster>(this);

    // publishers for landmarks
    // publish these with root namespace cuz we change the name of this node sometimes and we want
    // landmarks to always be at the same topic

    // create arena walls publisher
    rclcpp::QoS qos(10);
    qos.transient_local();
    walls_publisher_ = create_publisher<visualization_msgs::msg::MarkerArray>("/real_walls", qos);
...
private:
  ...
  void timer_callback()
  {
    ...
    auto [noisy_wheel_angle_left, noisy_wheel_angle_right] =
      noisy_diff_drive_->get_gt_wheel_angles();
    auto [encoder_wheel_angle_left, encoder_wheel_angle_right] =
      noisy_diff_drive_->get_encoder_wheel_angles();
    ...
    if (gt_obs_->did_collide()) {
      auto msg = std::format(
        "Collision ({}) Pose: {:D.3f} -> {:D.3f}.", count_msg.data, gt_pose_, new_gt_pose_);
    ...
    sensor_data_publisher_->publish(
      sensor_msg);  // clean data -> turtle_control -> blue/joint_states -> odometry
    ...
    std::shared_ptr<std_srvs::srv::Empty::Response>)
  {
    ...
    noisy_diff_drive_ = std::make_unique<NoisyDiffDrive>(
      get_parameter("wheel_radius").as_double(), get_parameter("track_width").as_double(),
      get_parameter("input_noise").as_double(), get_parameter("slip_fraction").as_double());
  ...
  {
    /*
     Each received wheel_cmd command sets the wheel velocities of the robot until the next wheel_cmd
     command is received. The wheel_cmd messages are integer values between -265 and 265 and are
     proportional to the maximum rotational velocity of the motor (see Specifications and A.8).
     */
    wheel_vel_left_ =
      std::clamp(msg->left_velocity, -motor_cmd_max_, motor_cmd_max_) * motor_cmd_per_rad_sec_;
    wheel_vel_right_ =
      std::clamp(msg->right_velocity, -motor_cmd_max_, motor_cmd_max_) * motor_cmd_per_rad_sec_;
  }
